RegisterNetEvent("Characters:Client:Logout", function()
	_inPickup = false
	_inLogout = false
	_doingMugshot = false
end)

RegisterNetEvent("Jail:Client:EnterJail", function()
	plsr.Sounds.Play:One("jailed.ogg", 0.075)
	if not IsScreenFadedOut() then
		DoScreenFadeOut(1000)
		while not IsScreenFadedOut() do
			Wait(10)
		end
	end

	local cellData = Config.Cells[math.random(#Config.Cells)]

	SetEntityCoords(PlayerPedId(), cellData.coords.x, cellData.coords.y, cellData.coords.z, 0, 0, 0, false)
	Wait(100)
	SetEntityHeading(PlayerPedId(), cellData.heading)
	_disabled = false

	Wait(1000)

	DoScreenFadeIn(1000)
	while not IsScreenFadedIn() do
		Wait(10)
	end
end)

AddEventHandler("Keybinds:Client:KeyUp:primary_action", function()
	if _inLogout then
		plsr.Callbacks:ServerCallback("Jail:Validate", {
			id = GlobalState[string.format("%s:Apartment", plsr.State.flags.ID)],
			type = "logout",
		}, function(state)
			if state then
				plsr.Characters:Logout()
			end
		end)
	end
end)

AddEventHandler("Jail:Client:RetreiveItems", function()
	plsr.Callbacks:ServerCallback("Jail:RetreiveItems")
end)

AddEventHandler("Jail:Client:CheckSentence", function()
	local jailed = plsr.State.character.Jailed
	if not jailed or GetCloudTimeAsInt() >= (jailed.Release or 0) then
		plsr.Notification:Info("Time Served")
	else
		if jailed.Duration >= 9999 then
			plsr.Notification:Info("You've Been Setenced To The 9's")
		else
			local months = math.ceil((jailed.Release - GetCloudTimeAsInt()) / 60)
			plsr.Notification:Info(
				string.format("You Have %s Months of Your %s Month Sentence Remaining", months, jailed.Duration)
			)
		end
	end
end)

AddEventHandler("Jail:Client:Released", function()
	if plsr.Jail:IsJailed() and plsr.Jail:IsReleaseEligible() then
		plsr.Callbacks:ServerCallback("Jail:Release", {}, function(s)
			if s then
				DoScreenFadeOut(1000)
				while not IsScreenFadedOut() do
					Wait(10)
				end

				plsr.Sounds.Play:One("release.ogg", 0.15)
				SetEntityCoords(
					PlayerPedId(),
					Config.Release.coords.x,
					Config.Release.coords.y,
					Config.Release.coords.z,
					0,
					0,
					0,
					false
				)
				Wait(100)
				SetEntityHeading(PlayerPedId(), Config.Release.heading)

				Wait(1000)

				DoScreenFadeIn(1000)
				while not IsScreenFadedIn() do
					Wait(10)
				end
			end
		end)
	end
end)

AddEventHandler("Polyzone:Enter", function(id, testedPoint, insideZones, data)
	if id == "prison-pickup" then
		_inPickup = true
	elseif id == "prison-logout" then
		_inLogout = true
		plsr.Action:Show("logout", "{keybind}primary_action{/keybind} Switch Characters")
	end
end)

AddEventHandler("Polyzone:Exit", function(id, testedPoint, insideZones, data)
	if id == "prison" and plsr.State.flags.loggedIn then
		if plsr.State.flags.inTrunk then
			plsr.Trunk:GetOut()

			while plsr.State.flags.inTrunk do
				Wait(1)
			end

			Wait(2000)

			plsr.Notification:Warn("Stop exploiting or you will be flighted")
			plsr.Callbacks:ServerCallback("Jail:Server:ExploitAttempt", 1)
		end

		if plsr.State.flags.myEscorter ~= nil then
			TriggerServerEvent("Escort:Server:ForceStop")

			while plsr.State.flags.myEscorter ~= nil do
				Wait(1)
			end

			plsr.Notification:Warn("Stop exploiting or you will be flighted")
			plsr.Callbacks:ServerCallback("Jail:Server:ExploitAttempt", 2)
		end

		if plsr.Jail:IsJailed() and not _doingMugshot then
			TriggerEvent("Jail:Client:EnterJail")
		end
	elseif id == "prison-pickup" then
		_inPickup = false
	elseif id == "prison-logout" then
		_inLogout = false
		plsr.Action:Hide("logout")
	end
end)

AddEventHandler("Jail:Client:StartWork", function()
	plsr.Callbacks:ServerCallback("Jail:StartWork")
end)

AddEventHandler("Jail:Client:QuitWork", function()
	plsr.Callbacks:ServerCallback("Jail:QuitWork")
end)

AddEventHandler("Jail:Client:MakeFood", function()
	plsr.Progress:Progress({
		name = "prison_action",
		duration = 12500,
		label = "Making Food",
		useWhileDead = false,
		canCancel = true,
		controlDisables = {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
		},
		animation = {
			anim = "dj",
		},
	}, function(status)
		if not status then
			plsr.Callbacks:ServerCallback("Jail:MakeItem", "food")
		end
	end)
end)

AddEventHandler("Jail:Client:MakeDrink", function()
	plsr.Progress:Progress({
		name = "prison_action",
		duration = 12500,
		label = "Making Drink",
		useWhileDead = false,
		canCancel = true,
		controlDisables = {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
		},
		animation = {
			anim = "dj",
		},
	}, function(status)
		if not status then
			plsr.Callbacks:ServerCallback("Jail:MakeItem", "drink")
		end
	end)
end)

AddEventHandler("Jail:Client:MakeJuice", function(self, data)
	plsr.Progress:Progress({
		name = "prison_action",
		duration = 12500,
		label = "Making Slushie",
		useWhileDead = false,
		canCancel = true,
		controlDisables = {
			disableMovement = true,
			disableCarMovement = true,
			disableMouse = false,
			disableCombat = true,
		},
		animation = {
			anim = "dj",
		},
	}, function(status)
		if not status then
			plsr.Callbacks:ServerCallback("Jail:MakeJuice", data.name)
		end
	end)
end)

AddEventHandler("Jail:Client:ViewInmates", function()
	TriggerServerEvent("MDT:Server:OpenDOCPublic")
end)
